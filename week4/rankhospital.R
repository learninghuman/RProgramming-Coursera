rankhospital <- function(state, outcome, num = "best") {
  ...
  if(outcome == "heart attack") {
    col_index <- 13
    new_df <- subset(outcome_df, outcome_df$State==state & outcome_df[[col_index]]!="Not Available", select=c(Hospital.Name, Hospital.30.Day.Death..Mortality..Rates.from.Heart.Attack))
    ...
    col_index <- 19
    new_df <- subset(outcome_df, outcome_df$State==state & outcome_df[[col_index]]!="Not Available", select=c(Hospital.Name, Hospital.30.Day.Death..Mortality..Rates.from.Heart.Failure))
    ...
    col_index <- 25
    new_df <- subset(outcome_df, outcome_df$State==state & outcome_df[[col_index]]!="Not Available", select=c(Hospital.Name, Hospital.30.Day.Death..Mortality..Rates.from.Pneumonia))
  ...
  if(num == "best") {
    as.vector(new_df[1,][[1]])
  } else if(num == "worst") {
    row <- nrow(new_df)
    as.vector(new_df[row,][[1]])
  } else {
    as.vector(new_df[num,][[1]])
  }
}
